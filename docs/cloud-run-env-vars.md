...
echo "YOUR_PROJECT_ID.firebaseapp.com" | gcloud secrets create FIREBASE_AUTH_DOMAIN --data-file=- --project=$PROJECT_ID
echo "YOUR_PROJECT_ID" | gcloud secrets create FIREBASE_PROJECT_ID --data-file=- --project=$PROJECT_ID
...
# echo "YOUR_PROJECT_ID.firebaseapp.com" | gcloud secrets versions add FIREBASE_AUTH_DOMAIN --data-file=- --project=$PROJECT_ID
# echo "YOUR_PROJECT_ID" | gcloud secrets versions add FIREBASE_PROJECT_ID --data-file=- --project=$PROJECT_ID
...
./setup-firebase-secrets.ps1 -ProjectId "YOUR_PROJECT_ID" -Region "us-central1" -ServiceName "gsdta-web" -GrantOnly
```
...
```yaml
--set-env-vars NEXT_TELEMETRY_DISABLED=1,NODE_ENV=production,NEXT_PUBLIC_AUTH_MODE=firebase,NEXT_PUBLIC_API_BASE_URL=/api,GOOGLE_CLOUD_PROJECT=YOUR_PROJECT_ID,NEXT_PUBLIC_USE_MSW=false,USE_GO_API=false
