...
|------------|--------|--------|-------------|------------------|---------|
| **Production** | `app.gsdta.com` | `main` | `gsdta-web` | `gsdta-prod` (existing: `playground-personal-474821`) | Live users |
...
gcloud beta billing projects link $QA_PROJECT_ID --billing-account=017449-E6C077-ADCB2A
...
# Note: Save the App ID from output (e.g., 1:575892687302:web:8f37a323522a6649678ac7)
...
{
  "projectId": "gsdta-qa",
  "appId": "1:575892687302:web:8f37a323522a6649678ac7",
  "storageBucket": "gsdta-qa.firebasestorage.app",
  "apiKey": "AIzaSyAFoXxhVhZKUL3mtntV04J7kBTcEidznvA",
  "authDomain": "gsdta-qa.firebaseapp.com",
  "messagingSenderId": "575892687302"
}
```
...
- **Production secrets** → `playground-personal-474821` (existing)
...
```bash
export QA_PROJECT_ID="gsdta-qa"
export PROD_PROJECT_ID="playground-personal-474821"

# Create secrets in QA project (use values from SDK config in step 3.1)
echo -n "AIzaSyAFoXxhVhZKUL3mtntV04J7kBTcEidznvA" | gcloud secrets create FIREBASE_API_KEY --data-file=- --project=$QA_PROJECT_ID
...
echo -n "1:575892687302:web:8f37a323522a6649678ac7" | gcloud secrets create FIREBASE_APP_ID --data-file=- --project=$QA_PROJECT_ID
...
for SECRET in FIREBASE_API_KEY FIREBASE_AUTH_DOMAIN FIREBASE_PROJECT_ID FIREBASE_APP_ID; do
  gcloud secrets add-iam-policy-binding $SECRET \
    --member="serviceAccount:915562784753-compute@developer.gserviceaccount.com" \
    --role="roles/secretmanager.secretAccessor" \
    --project=$QA_PROJECT_ID
done
...
jobs:
  ...
  deploy:
    ...
    with:
      ...
      gcp_project_id: playground-personal-474821
      secrets_project_id: playground-personal-474821  # Prod secrets in prod project
      ...
      - name: Set environment config
        ...
        run: |
          if [ "${{ inputs.environment }}" = "production" ]; then
            ...
            echo "firebase_project_id=playground-personal-474821" >> $GITHUB_OUTPUT
            echo "secrets_project_id=playground-personal-474821" >> $GITHUB_OUTPUT
          else
...
|-------------|-------------|-----------------|
| `GCP_SA_KEY` | Service account JSON key for `gsdta-web-ci@playground-personal-474821.iam.gserviceaccount.com` | ✅ Yes (existing) |
...
# List secrets
gcloud secrets list --project=playground-personal-474821

# Switch between Firebase projects
firebase use gsdta-qa      # QA
firebase use playground-personal-474821  # Production
```
...
| **GCP Project (Cloud Run)** | `playground-personal-474821` | `playground-personal-474821` |
| **Firebase/Secrets Project** | `gsdta-qa` | `playground-personal-474821` |
