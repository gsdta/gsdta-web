...
Grant the missing Secret Manager role with the commands below. They bind the `roles/secretmanager.secretAccessor` role at the project level so the workflow can read all Firebase secrets that live in `playground-personal-474821`.

## Prerequisites
- gcloud CLI installed and authenticated to the target project (Owner/Editor or a custom role with IAM write on the project)
- The GitHub Actions service account exists (example below uses: `gsdta-web-ci@playground-personal-474821.iam.gserviceaccount.com`)
...
```cmd
set PROJECT_ID=playground-personal-474821
